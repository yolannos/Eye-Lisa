...
def plot_house(chm, house_coordinates):
    ...
    '''
    data = rasterio.open("lidar_chm.tif")
    ...
    # with fiona.open("/home/yolann/Documents/becode/projects/LIDAR-PROJECT/3d_objects/Shapefile/GRBGebL1D2.shp", "r") as shapefile:
        
    #     for feature in shapefile.filter(bbox=(l, b, r, t)):
            
    #         ### WITHOUT TRACE
    #         _ = feature["geometry"]["coordinates"][0]
    #         data_trace = np.array(_)
    #         height = np.full(len(data_trace), feature["properties"]["HN_MAX"] )
    #         base = np.full(len(data_trace), 0.5)
    #         df_up = pd.DataFrame(data_trace, columns=["x","y"])
    #         df_up["z"] = height.tolist()
    #         df_down = pd.DataFrame(data_trace, columns=["x","y"])
    ...
    #         fig.add_scatter3d(x=df['x'], y=df['y'], z=df["z"], surfaceaxis= -1, mode='lines',showlegend=False, line=dict( width=5))
    #         print(df)
